...
OTel also need to be configured to enable specific receivers, processors and exporters.  We will do that in [configs/otel-collector.yaml](https://github.com/panyam/onehub/blob/PART11_TRACING/configs/otel-collector.yaml).
...
So far we have the right systems where we can visualize, consume signals etc.  However our services are still not updated to emit the signals to OTel.   Here we will integrate with the (Golang) client SDK in various parts of our code.  The [SDK](https://opentelemetry.io/docs/languages/go/) documentation is a fantastic place to first familiarize with some of the concepts.   

Briefly we have:

### [Resources](https://opentelemetry.io/docs/languages/go/resources/)

Resources are the entity which is producing the signal.  In our case the scope of the resource is the binary hosting the services.   Currently we have a single resource for the entirety of the Onehub service but this could be split up later on. 
...
Only override we had provided was the `OTEL_RESOURCE_ATTRIBUTES: service.name=onehub.backend,service.version=0.0.1` environment variable for the `onehub` service in [docker-compose.yml](https://github.com/panyam/onehub/blob/PART11_TRACING/docker-compose.yml).

### [Contexts Propogation](https://opentelemetry.io/docs/concepts/context-propagation/)

Context propogation is a very important topic in observability.  The various pillars are exponentially power when we can correlate signals from each of the pillars in identifying issues with our system.   Think of contexts as extra bits of data that can be tied to the signals that can be "joined" in some unique way to tie the various signals related to a particular group (say a reqeust).

### Providers/Exporters

For each of the signals, OTel provides a Provider interface (eg TracerProvider for exporting spans/traces, MeterProvider for exporting metrics, LoggerProvider for exporting logs and so on).  For each of these interfaces there can be several implementations, eg Debug provider for sending to stdout/err streams, OTel provider for exporting to an OTel endpoint, or even directly via a [variety of exporters](https://opentelemetry.io/ecosystem/registry/?component=exporter&language=go).   However in our case we want to defer the choice of any vendors out of our services and instead send all signals to the OTel collector running in our environment.
...
* Exporting signals to an OTel collector - [cmd/backend/otelcol.go](https://github.com/panyam/onehub/blob/PART11_TRACING/cmd/backend/otelcol.go)
...
As an example our gateway service leverages this channels in the following way.  Instead of 

```go
	http.ListenAndServe(gw_addr, mux)
```

we now have (pay attention to **lines 9-14** where the server shutdown is watched for in a separate goroutine and **line 15** where the server exit error if any is sent back via the passed "notification" channel):
...
In the previous  the http.Server instance used to start the grpc Gateway is using a custom handler - the `http.Handler` in the [Otel HTTP](go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp) package.  This Handler takes an existing http.Handler instance and decorates it with the OTel context and ensuring its propogation to any other downstreams that are called.  Our handler is simple:
...
What we have done is first create a `client` (**Line 6**) that acts as a client factory for our gRPC server.  The client is pretty simple with the only addition of the grpc ClientHandler being used to create the connection.   This ensures that the context in the current trace that began on a new http request is now propogated to the gRPC server via this Handler.
