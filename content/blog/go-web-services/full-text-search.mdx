...
<center> <img src="{{.Context.PathPrefix}}/static/images/part9/current-search-architecture.svg" height={600} /> </center>
...
Before diving into the proposed architecture, let us discuss what we want from our search system (both functionally and operationally).  This influence our choice of design, architecture and technologies.
...
2. Searching messages/topics (by different fields) must be fast but we want to vary how quickly an action in (1) is reflected in a respective search result.
...
Given our requirements we can start with a proposed architecture that allows an index that can be built incrementally and asynchronously:

<center> <img src="{{.Context.PathPrefix}}/static/images/part9/proposed-search-architecture.svg" height={600} /> </center>
