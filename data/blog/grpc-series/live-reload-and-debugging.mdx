...
<CodeEmbed
  ...
  language="yaml"
  url="https://raw.githubusercontent.com/panyam/onehub/PART7_AIR/.air.toml"
  height="300px"
...
* `build.exclude_dir` - Specifies which files/folders are NOT to be watched.  This will ensure are do not over-trigger rebuilding and restarting our binary.
* `build.exclude_regex` - Specifies which file patterns are to be ignored.

Other than this by default all files and sub directories in the project's path will be watched.
...
(_-c_ is optional and by default air uses .air.toml as its config file)

With this Air will first build the binary (`./tmp/main`) and execute it.  Try changing one of the source files and see air rebuild and restart the binary!  (Note - simply `touch`ing a file willnot work as air compares file contents too for changes to count).
...
* Note that we are adding the gen folder as a Volume mapping.  So if protos change make sure to regenerate the artifacts (`buf generate`) and restart (docker compose).

## Enable Debugging

When we run the service outside docker it was easy to bring it up via VSCode or any other IDE to debug it interactively.  However doing so for a binary running inside a docker image needs a few small changes.   Delve is the standard tool for enable remote attaching/debugging of Go applications.   Thankfully we had already Delve in our dependencies above.
...
Delve is now running the app with a debug server running port 9091.  GO IDEs can now connect to this port and interactively debug the binary.
