...
Ok third time lucky.  This time we ARE really almost there!  With the data base up (`docker compose up`) and service running (`go run cmd/server.go`) we can start calling methods on our service:
