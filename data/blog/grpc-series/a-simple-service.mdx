...
[gRPC](https://grpc.io/) is a great framework for creating (micro) services in many environments.  gRPC offers a clean human readable source-of-truth of your service specs without needing further tools to understand them.    What makes the gRPC ecosystem shine is its plugin facility for extending it on several fronts.   With plugins you can:
...
## Testing calls to the server

Now it is time to test our server.  Note the grpc server is *not* a REST endpoint. So curl would not work (we will cover this in [Part 2](/blog/grpc-series/rest-http-gateway/)).  We can make calls against the server in a couple of ways - using a CLI utility (much like curl for REST/http services) or by using the clients generated by the protoc tool.  Even better, we can also make client calls from other langauges - if we had opted to generate libraries targetting those languages too.
...
### Prgrammatically calling the server
...
That was a lot to cover but we made it.   Even though it was a basic example, (hopefully) it set a good foundation for the topics in the rest of the series.   In this guide we:
