...
Ultimately our cli will make http calls to the rest endpoint (generated by the grpc-gateway plugin in [part 2](/grpc-series/rest-http-gateway/) of this series).   So a easy way to make http calls that follows our conventions (auth headers, json in/json out, etc) is useful.  There are [excellent libraries](https://awesome-go.com/http-clients/) for wrapping the native Go http library.  But since our requirements are simple, let us add our own wrapper for use in here.  Our goals are simple:
...
In this deep-dive we built a basic CLI for accessing our OneHub (chat) service.  We will build upon this CLI in future posts for more advanced use cases (shell command integration, subscribing to streams etc).
